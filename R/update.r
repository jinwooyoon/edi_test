...
NewEdiUpdate<-function(ediData,
  ...
  sql <- " SELECT COUNT(*) AS NEW_CODE_COUNT FROM @table_name AS new_code
        WHERE new_code.concept_code not in (SELECT concept_code from EDI_OMOP.@voca_schema.@existing_voca_table)"
  ...
  sql <- "INSERT INTO EDI_OMOP.@voca_schema.@existing_voca_table
  ...
  ##Deleted code update
  sql <- "UPDATE EDI_OMOP.@voca_schema.@existing_voca_table
        SET VALID_END_DATE = '@valid_end_date', INVALID_REASON = 'D'
        FROM EDI_OMOP.@voca_schema.@existing_voca_table as existing_table
  ...
  sql <- " SELECT COUNT(*) AS DELETED_CODE_COUNT FROM EDI_OMOP.@voca_schema.@existing_voca_table AS existing_table
